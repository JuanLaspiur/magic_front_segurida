<template>
    <div class="inicio_container q-px-sm">
      <div class="inicio q-my-md items-center justify-center">
        <div v-if="width < 500" class="shadow-1" style="border-radius: 10px;overflow: hidden; height: 200px; width: 100%;">
          <q-carousel
        ...
        <!-- Planes premiums -->
        <div class="text-grey-10 q-mt-md text-h6 q-py-sm text-bold q-pl-sm">Planes premiums</div>
        <div v-if="allQuedadasPremium.length && width < 500" class="q-mt-xs" style="margin-bottom: 10px;">
          <ContainerCards
        ...
        <div class="text-grey-10 text-h6 q-py-sm q-pl-sm text-bold">Busca un plan cercano a ti</div>
        ...
        <div class="text-grey-8 text-h7 q-mt-md q-pl-sm">Selecciona la categoría</div>
      ...
      <div class="text-grey-10 text-h6 q-py-sm q-pl-sm text-bold">Proximos planes </div>
        <ContainerAll
        ...
        <div style="display: flex; justify-content: center; align-items: center;">
          <q-btn @click="$router.push('/planes')" color="primary">Ver todos los planes</q-btn>
        </div>
        <div style="display: flex; justify-content: center; align-items: center; margin-top: 50px; margin-bottom: 50px;">
    <q-card>
        <q-card-section class="q-pa-md">
            <!-- Contenido de la encuesta -->
            <h5 style="text-align: center; margin-bottom: 20px;">¿Qué prefieres?</h5>
            <div>
                <input type="checkbox" id="opcion1" name="opcion1" value="Opción A">
                <label for="opcion1">1. Opción A</label><br>
                <input type="checkbox" id="opcion2" name="opcion2" value="Opción B">
                <label for="opcion2">2. Opción B</label><br>
                <input type="checkbox" id="opcion3" name="opcion3" value="Opción C">
                <label for="opcion3">3. Opción C</label><br>
            </div>
        </q-card-section>
        <q-card-actions align="center">
            <q-btn color="primary" label="Enviar" @click="enviarEncuesta"></q-btn>
        </q-card-actions>
    </q-card>
</div>
        <div v-if="width < 500" class="row q-mt-xl justify-center items-center cursor-pointer shadow-1 inicio_banner" style="border-radius: 10px; overflow:hidden; height: 140px; width: 100%;" >
          <img src="banner_prueba.png" style="width:100%; height: 100%; object-fit: cover;" @click="irRuta('http://www.ultrareformas.es/')" />
        </div>
        <div v-else class="row justify-center items-center q-mt-xl cursor-pointer shadow-1 inicio_banner" style="border-radius: 10px; overflow:hidden" >
        ...
        <div v-if="quedadas.length && width < 500" class="q-mt-sm" style="margin-bottom: 10px;">
          <ContainerAll
          :chunck="quedadas"
          :thumbStyleScroll="null"
          :baseuQuedada="baseuQuedada"
...
export default {
  components: { SideBar, RightSidebar, CarouselForLastPlans, ContainerCards, ContainerAll, CarouselWithoutImg },
  data () {
    return {
      autoplay: true,
      perfile: null,
      ...
      slide: 1,
      encuestas: [
        { pregunta: '¿Qué prefieres?', opciones: ['Opción A', 'Opción B', 'Opción C'] },
        { pregunta: '¿Cuál es tu color favorito?', opciones: ['Rojo', 'Azul', 'Verde'] }
      ],
  ...
  methods: {
    ...
    siguienteEncuesta () {
      this.indiceEncuestaActual = (this.indiceEncuestaActual + 1) % this.encuestas.length
    }
  }
}
</script>
