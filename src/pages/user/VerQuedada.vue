  ...
  <q-dialog v-model="modalOpen" persistent>
    <q-card>
            ...
            <tr v-for="solicitud in listaDeSolicitudes" :key="solicitud.id">
              <td>
                <div class="row items-center">
                  <q-avatar v-if="solicitud.avatar" :src="solicitud.avatar"></q-avatar>
      ...
      <q-card-actions align="right">
        <q-btn flat label="Salir" color="primary" @click="modalOpen = false" />
      </q-card-actions>
    </q-card>
  </q-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async aceptarSolicitud (solicitud) {
      await this.$api.put(`aceptarSolicitudPremium/${this.quedada._id}`, { user_id: solicitud._id, aceptado: true })
    },

    async rechazarSolicitud (solicitud) {
      await this.$api.put(`aceptarSolicitudPremium/${this.quedada._id}`, { user_id: solicitud._id, aceptado: false })
    },
    async addImg () {
      this.$q.loading.show({
