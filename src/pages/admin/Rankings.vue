<template>
  <div>
    <div class="text-center text-h6 q-my-sm">Rankings de Usuarios</div>
    <q-separator />
    <!-- <div class="text-center text-subtitle1 q-my-sm">Ranking de usuarios más activos</div>
    <q-table
      :data="usersByTime"
      :columns="columns"
      row-key="name"
    ...
    >
      ...
      <template v-slot:body-cell-animalImg="props">
        <q-td :props="props">
          <q-avatar size="40px">
            <q-img
              :src="'animales/' + props.row.animalInfo.img"
              class="full-height"
            />
          </q-avatar>
        </q-td>
      ...
      <template v-slot:body-cell-medallas="props">
        <q-td :props="props">
          <div v-if="props.row.eventosAsistidos.length && props.row.eventosCreados.length">
            <div
              v-if="
                (props.row.eventosAsistidos.length >= 5 &&
                  props.row.eventosCreados.length >= 10 &&
                  props.row.eventosAsistidos.length < 10 &&
                  props.row.eventosCreados.length < 25) ||
                (props.row.eventosCreados.length >= 25 && props.row.eventosAsistidos.length < 10) ||
                (props.row.eventosCreados.length < 25 && props.row.eventosAsistidos.length >= 10)
              "
            >
              Bronce
            ...
            <div
              v-if="
                (props.row.eventosAsistidos.length >= 10 &&
                  props.row.eventosCreados.length >= 25 &&
                  props.row.eventosAsistidos.length < 20 &&
                  props.row.eventosCreados.length < 50) ||
                (props.row.eventosCreados.length >= 50 && props.row.eventosAsistidos.length < 20) ||
                (props.row.eventosCreados.length < 50 && props.row.eventosAsistidos.length >= 20)
              "
            >
              Plata
            ...
            <div
              v-if="
                (props.row.eventosAsistidos.length >= 20 &&
                  props.row.eventosCreados.length >= 50 &&
                  props.row.eventosAsistidos.length < 50 &&
                  props.row.eventosCreados.length < 100) ||
                (props.row.eventosCreados.length >= 100 && props.row.eventosAsistidos.length < 50) ||
                (props.row.eventosCreados.length < 100 && props.row.eventosAsistidos.length >= 50)
              "
            >
              Oro
            </div>
            <div v-if="props.row.eventosAsistidos.length >= 50 && props.row.eventosCreados.length >= 100">
              Platino
            </div>
    ...
    <div class="text-center text-subtitle1 q-my-sm">Ranking de usuarios creadores de planes</div>
    <q-table
      :data="usersByCreate.filter((persona) => persona.enable === true)"
      :columns="columns"
      row-key="name"
      :pagination.sync="pagination"
      class="q-ma-md"
    ...
    <div class="text-center text-subtitle1 q-my-sm">Ranking de usuarios asistentes a planes</div>
    <q-table
      :data="usersByAsist.filter((persona) => persona.enable === true)"
      :columns="columns"
      row-key="name"
      :pagination.sync="pagination"
      class="q-ma-md"
    ...
    async getUsers () {
      this.$api.get('all_user?order=1?creados=1?asistidos=1').then((res) => {
        if (res) {
          ...
          for (let i = 0; i < this.usersByCreate.length; i++) {
            this.getTiempo(res.usersByCreate[i]._id, i, res.usersByCreate)
          }
          for (let i = 0; i < this.usersByAsist.length; i++) {
            this.getTiempo(res.usersByAsist[i]._id, i, res.usersByAsist)
          }
        }
